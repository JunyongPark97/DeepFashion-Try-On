...
def generate_discrete_label(inputs, label_nc,onehot=True,encode=True):
    ...
    for input in inputs:
        ...
        pred = np.squeeze(input.data.max(1)[1].cpu().numpy(), axis=0) # 14개의 레이어에 각각 그 해당 숫자가 될 확률이 나옴 -> 그중(14개중)최대 확률값을 뽑아서 대표로 만듬 -> pred batch
...
class Pix2PixHDModel(BaseModel):
    ...
    def forward(self,label,pre_clothes_mask,img_fore,clothes_mask,clothes,all_clothes_label,real_image,pose,mask):
        ...
        G_in=torch.cat([img_hole_hand,masked_label,real_image*clothes_mask,skin_color,self.gen_noise(shape)],1)  #####????input 이좀 다른듯
